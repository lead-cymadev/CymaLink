  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-slate-100">
      <div className="flex flex-col items-center space-y-4">
        ...
        <p className="text-slate-600">{t('loading.dashboard', 'Cargando dashboard...')}</p>
      </div>
    </div>
  );
}
    ...
    <div className="flex items-center justify-center min-h-screen bg-slate-100">
      <div className="bg-white p-8 rounded-xl shadow-sm max-w-md w-full mx-4">
        <div className="text-center">
          ...
          <h3 className="text-lg font-semibold text-slate-800 mb-2">{t('error.title', 'Error al Cargar Datos')}</h3>
...
export function StatusBadge({ status }: { status?: string }) {
  ...
  return (
    <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold ${styles}`}>
      {status ?? t('status.unknown', 'Sin estado')}
    </span>
  );
}
...
export function DashboardTopBar({
  ...
  onRefresh?: () => void;
  onMenuToggle?: () => void;
  navItems?: Array<{ label: string; value?: string; icon: ComponentType<SVGProps<SVGSVGElement>>; badge?: string; active?: boolean; onClick?: () => void }>;
}) {
  const t = useI18n();
  return (
    <header className="border-b border-slate-200 bg-white/95 backdrop-blur">
      <div className="flex flex-col gap-4 px-4 py-5 lg:flex-row lg:items-center lg:justify-between lg:px-8">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.35em] text-blue-600">{t('app.name', 'CymaLink')}</p>
        ...
        <div className="flex items-center gap-3">
          <div className="hidden items-center rounded-xl border border-slate-200 bg-white px-3 py-2 shadow-sm sm:flex">
            ...
            <input
              type="search"
              placeholder={t('search.placeholder', 'Buscar sitios o dispositivos')}
...
export function Sidebar({
  ...
  roleLabel: UserRole;
  onLogout: () => void;
  navItems: Array<{ label: string; value?: string; icon: ComponentType<SVGProps<SVGSVGElement>>; badge?: string; active?: boolean }>;
  onItemClick?: (value: string) => void;
  onClose?: () => void;
}) {
  const t = useI18n();
  const roleText = roleLabel.toLowerCase() === 'admin' ? t('sidebar.role.admin', 'admin') : t('sidebar.role.user', 'usuario');
  return (
    <aside className="hidden w-72 flex-col border-r border-slate-200 bg-white/95 pb-6 backdrop-blur lg:flex">
      <div className="flex h-full flex-col pb-6">
        <div className="flex items-center justify-between px-6 py-6">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.3em] text-blue-600">{t('app.name', 'CymaLink')}</p>
            <h2 className="text-xl font-bold text-blue-900">{t('app.networkCenter', 'Network Center')}</h2>
          </div>
          <span className="rounded-full bg-red-50 px-3 py-1 text-xs font-semibold text-red-600 capitalize">{roleText}</span>
        </div>
